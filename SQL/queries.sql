...
-- 2. Top Rated Events (minimum 10 feedback)
...
SELECT s1.event_id, s1.session_id, s2.session_id AS conflicting_session
FROM Sessions s1
JOIN Sessions s2 ON s1.event_id = s2.event_id AND s1.session_id <> s2.session_id
WHERE s1.start_time < s2.end_time AND s1.end_time > s2.start_time;

-- 16. Unregistered Active Users (last 30 days, no registrations)
SELECT *
FROM Users u
...
SELECT e.title, COUNT(DISTINCT r.user_id) AS registration_count, AVG(f.rating) AS avg_rating
FROM Events e
